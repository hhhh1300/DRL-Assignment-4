...
class Actor(nn.Module):
    ...
    def forward(self, obs):
        x = self.fc(obs)
        mu = torch.tanh(self.mu(x)) * self.action_scale
...
class Agent(object):
    ...
    def __init__(self):
        ...
        self.agent = PPO_Agent(5, 1) # obs_dim=3 for Pendulum, act_dim=1 for action space
        self.agent.actor.load_state_dict(torch.load("models/ppo_actor_ep300.pth"))
        self.agent.critic.load_state_dict(torch.load("models/ppo_critic_ep300.pth"))
